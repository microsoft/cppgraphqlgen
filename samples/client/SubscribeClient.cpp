...
template <>
subscribe::TaskState ModifiedArgument<subscribe::TaskState>::convert(const response::Value& value)
{
	if (!value.maybe_enum())
	{
	...
	return static_cast<subscribe::TaskState>(itr - s_namesTaskState.cbegin());
}

template <>
std::future<service::ResolverResult> ModifiedResult<subscribe::TaskState>::convert(service::FieldResult<subscribe::TaskState>&& result, ResolverParams&& params)
{
	return resolve(std::move(result), std::move(params),
		[](subscribe::TaskState&& value, const ResolverParams&)
		{
...
template <>
subscribe::CompleteTaskInput ModifiedArgument<subscribe::CompleteTaskInput>::convert(const response::Value& value)
{
...
{
	auto argInput = service::ModifiedArgument<subscribe::CompleteTaskInput>::require("input", params.arguments);
	std::unique_lock resolverLock(_resolverMutex);
	auto directives = std::move(params.fieldDirectives);
